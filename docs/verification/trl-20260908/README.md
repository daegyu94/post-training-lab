...
For the separate GPU checks after migration, see the [integration verification record](../integration-20260908/README.md).
...
이 문서의 결과는 통합 후 소형 모델 smoke와 구분합니다.
...
현재 실행 준비와 설정은 [TRL 가이드](../../backends/trl.md)를 따릅니다.
아래 기록은 당시의 모델·환경·조건에 한정되며, 이 문서 정리 과정에서 GPU 재실행한 결과가 아닙니다.

## Evidence

[실행 상태](execution.json), [ZeRO-3 summary](small-zero3-summary.json), [첫 30B FSDP2 OOM 기록](qwen30b-fsdp2-oom.txt)을 함께 보존합니다.
[Qwen3 개별 full SFT driver 기록](qwen30b-single-full-kernel.txt)은 해당 실행 시간대의 로그만 포함합니다.
