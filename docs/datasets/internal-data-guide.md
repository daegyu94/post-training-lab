...
이 문서는 사내 LLM 서비스의 trace와 benchmark를 TRL과 Megatron 실습에서 사용하는 학습·검증 데이터로 바꾸고, 학습에 노출되지 않는 test set과 정답을 별도로 관리하는 기준을 설명합니다.
...
현재 실습의 service trace converter는 `system`, `user`, `assistant` role만 허용합니다.
