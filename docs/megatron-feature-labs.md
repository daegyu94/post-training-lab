...
0.5B paired run에서 checkpoint/restart correctness의 제한된 evidence는 확보했지만, crash·power-loss durability는 아직 검증하지 않았습니다.
raw Megatron rank log parser로 full 0.5B 2-node 6-step logs의 step 3–6 steady median을 계산하면 overlap off `315.2 ms`, on `281.0 ms`였지만 single pair이므로 stable benchmark나 speedup claim으로 보고하지 않습니다.
양 노드의 NCCL collective correctness와 host RoCE `ib_write_bw`는 별도 prerequisite smoke입니다.
...
Raw rank log는 within-run warmup을 명시해 다음처럼 파싱합니다. `--feature`, `--variant`, `--run-index`와 log path를 반드시 지정하며, parser의 기본 warmup은 첫 2개 iteration입니다.
...
2026-09-08에 `data/megatron-mid-async-rank{0,1}.log`와 `data/megatron-mid-sync-rank{0,1}.log`의 paired run을 확인했습니다.
두 run 모두 Qwen/Qwen2.5-0.5B-Instruct, HuggingFaceH4/no_robots, BF16 full mode, DP=2, global batch 2, micro batch 1, 8 optimizer steps, save interval 4를 사용했고 process exit 0으로 완료했습니다.
Async run은 iteration 4에서 save를 schedule한 뒤 iteration 5–8을 계속 수행했으며, iteration 8의 pending-save finalization/backpressure timer는 `60117.55–60117.59 ms`였습니다.
Sync run의 save timer는 iteration 4에서 `65191.37–65191.40 ms`, iteration 8에서 `61735.60–61735.66 ms`였습니다.
이 수치는 단일 paired run의 rank-log 관측값이며 async/sync speedup이나 일반적인 I/O 성능을 뜻하지 않습니다.

`data/megatron-resume-async4-rank{0,1}.log`는 async run의 `results/megatron-mid-async/checkpoints/iter_0000004`를 읽어 새 output `results/megatron-resume-from-async4/checkpoints`에 저장했습니다.
로그에는 distributed optimizer와 scheduler state를 포함한 load 성공, iteration 4에서 시작해 5–8을 수행한 사실과 iteration 8 저장이 남아 있습니다.
Resume의 step 5–8 LR/loss/grad는 연속 run과 print precision에서 일치했고, `data/resume-tensor-comparison.log`의 사전 선택된 norm/bias tensor 4개도 `mismatch []`였습니다.
이는 선택된 4개 model tensor의 equality만 확인한 것이며, whole model·optimizer state의 bitwise equality나 crash/power-loss durability를 입증하지 않습니다.
...
Rank 1의 steady step 3–6 median은 sequence-parallel off `239.85 ms`, on `252.1 ms`로 관측되어 on이 약 5.1% 느렸습니다.
이는 한 paired run의 descriptive result일 뿐 stable regression, speedup 또는 model-quality claim이 아닙니다.
Rank 1의 2-step 이후 max allocated memory는 off `4.8374 GiB`, on `4.8175 GiB`로 on이 약 `0.0199 GiB` 낮았지만, 이 작은 차이를 일반적인 memory saving으로 해석하지 않습니다.
