...
이 JSON을 실제로 어디서 읽는지는 목적에 따라 다릅니다 — 로컬에서만 보면 같은 노드의 `profiling_lab.framework_metrics_textfile`이 이를 Node Exporter textfile collector로 재발행해 Grafana에서 바로 보이고([Local Viewing](observability.md#local-viewing)), 외부 공유가 필요하면 node agent가 SSH reverse tunnel로 별도 collector에 전송합니다(Observatory).
