...
이 문서는 사내 LLM 서비스의 trace와 benchmark를 Megatron Bridge의 SFT 입력으로 바꾸고, 학습에 노출되지 않는 test set과 정답을 별도로 관리하는 기준을 설명합니다.
...
현재 Qwen용 `ChatSFTPreprocessingConfig`와 실습 converter는 `system`, `user`, `assistant` role만 허용합니다.
...
`ChatSFTPreprocessingConfig(loss_mode="assistant")`가 Qwen tokenizer의 chat template을 적용하고 assistant token에 loss를 계산합니다.
