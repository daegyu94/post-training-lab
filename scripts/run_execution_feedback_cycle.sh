...
"$PYTHON" -m execution_feedback.generate --model-dir "$SFT_CHECKPOINT" \
  ...
  --seed 42 --temperature 0.8 --top-p 0.95 --output "$WORK_DIR/train-candidates.jsonl"
...
common_train=(--max-steps 64 --per-device-batch-size 1 --gradient-accumulation-steps 4 --seed 42)
...
for variant in A B C D; do
  ...
  "$PYTHON" -m execution_feedback.generate --model-dir "${models[$variant]}" \
    ...
    --seed 42 --temperature 0 --output "$WORK_DIR/test-candidates-$variant.jsonl"
  "$PYTHON" -m execution_feedback.evaluate --tasks "$WORK_DIR/data/tasks.jsonl" \
