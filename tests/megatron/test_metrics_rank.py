...
@pytest.mark.parametrize('stage', ['all', 'train'])
def test_comparison_reads_last_rank_logs_on_their_node(tmp_path, node_rank, stage):
    ...
    assert len(compared) == (node_rank if stage == 'all' else 0)
    if node_rank and stage == 'all':
        args = compared[0]
