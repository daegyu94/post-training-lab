...
PRESETS = {
    "ultrachat": PublicDatasetSpec("ultrachat", "HuggingFaceH4/ultrachat_200k", "default", "train_sft", "mit", "messages", "8049631c405ae6576f93f445c6b8166f76f5505a"),
    "no_robots": PublicDatasetSpec("no_robots", "HuggingFaceH4/no_robots", "default", "train", "cc-by-nc-4.0", "messages", "e6f9a4ac5c37faeb744ba9ecf0473184d7f8105b"),
    "self_oss": PublicDatasetSpec("self_oss", "bigcode/self-oss-instruct-sc2-exec-filter-50k", "default", "train", "odc-by", "instruction,response", "356bb069eee815daa6e23e9a282eeefe1490ad44"),
    "xlam": PublicDatasetSpec("xlam", "Salesforce/xlam-function-calling-60k", "dataset", "train", "cc-by-4.0", "query,tools,answers (JSON strings)", "26d14ebfe18b1f7b524bd39b404b50af5dc97866", gated=True),
    "swe_trajectories": PublicDatasetSpec("swe_trajectories", "nebius/SWE-agent-trajectories", "default", "train", "cc-by-4.0 plus per-repository terms", "instance_id,model_name,target,trajectory,exit_status,generated_patch,eval_logs", reference_only=True),
}
...
def adapt_row(spec: PublicDatasetSpec, row: dict[str, Any], ordinal: int = 0) -> dict[str, Any]:
    ...
    if spec.key in {"ultrachat", "no_robots"}:
        ...
        messages = [{"role": "user", "content": _text(row.get("instruction"), "instruction")}, {"role": "assistant", "content": _text(row.get("response"), "response")}]
    elif spec.key == "xlam":
        ...
        messages = [
            {"role": "system", "content": "Available tools (JSON): " + _canonical_json(tools, "tools") + "\nReturn the selected call(s) as JSON."},
            {"role": "user", "content": _text(row.get("query"), "query")},
            {"role": "assistant", "content": _canonical_json(answers, "answers")},
        ]
    else:
        raise ValueError(f"no adapter for {spec.key}")
    return {"prompt_id": prompt_id, "messages": messages, "provenance": {"dataset": spec.dataset_id, "preset": spec.key, "source_id": prompt_id, "adapter_version": ADAPTER_VERSION}}


def select_bounded(rows: Iterable[dict[str, Any]], spec: PublicDatasetSpec, train_count: int, eval_count: int, seed: int, max_scan: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    if train_count < 1 or eval_count < 1 or max_scan < 1:
        raise ValueError("train-count, eval-count, and max-scan must be positive")
    train, evaluation, seen_ids, seen_groups = [], [], set(), set()
    for ordinal, source_row in enumerate(rows):
        if ordinal >= max_scan:
            break
        row = adapt_row(spec, source_row, ordinal)
        key = row["prompt_id"]
        group = hashlib.sha256(json.dumps([message for message in row["messages"] if message["role"] != "assistant"], ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
...
def iter_hub_rows(spec: PublicDatasetSpec, revision: str) -> Iterator[dict[str, Any]]:
    ...
    kwargs = {"split": spec.source_split, "revision": revision, "streaming": True}
    dataset = load_dataset(spec.dataset_id, spec.config, **kwargs) if spec.config else load_dataset(spec.dataset_id, **kwargs)
    yield from dataset
...
def write_public_jsonl(output_dir: Path, spec: PublicDatasetSpec, revision: str, train: list[dict[str, Any]], evaluation: list[dict[str, Any]], seed: int, max_scan: int) -> dict[str, Any]:
    ...
    selection_hash = hashlib.sha256(json.dumps({"train": train_ids, "validation": eval_ids}, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    manifest = {"manifest_version": 1, "adapter_version": ADAPTER_VERSION, "preset": spec.key, "dataset": spec.dataset_id, "config": spec.config, "dataset_revision": revision, "source_split": spec.source_split, "validation_policy": "deterministic holdout from source train split; source test split not read", "source_schema": spec.source_schema, "license": spec.license, "seed": seed, "max_scan": max_scan, "train_count": len(train), "eval_count": len(evaluation), "train_prompt_ids": train_ids, "eval_prompt_ids": eval_ids, "source_selection_sha256": selection_hash, "files": {name: {"path": path.name, "sha256": _sha256(path)} for name, path in paths.items()}}
