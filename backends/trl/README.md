...
[Setup 1 가이드](../../docs/backends/trl/setup1.md)에서 설치 → 데이터·모델 준비 → 실행 → 결과 확인 → adapter 재로딩 순서로 진행하세요.
[실험 기록](../../docs/backends/trl/experiment-result.md)은 128개 training conversation, 15개 held-out conversation과 20 optimizer steps를 사용합니다.
...
[Setup 2 가이드](../../docs/backends/trl/spark-cluster.md)에서 runtime·통신 확인 → 모델·데이터 준비 → 두 노드 실행 → 결과 확인 순서로 진행하세요.
...
이번 실행의 조건·수치·실패 원인은 [Training Verification](../../docs/backends/trl/training-verification.md)에 기록했습니다.
이전 실행은 [Setup 2 기존 기록](../../docs/backends/trl/spark-cluster.md#verified-integration-smoke)에 보존합니다.
