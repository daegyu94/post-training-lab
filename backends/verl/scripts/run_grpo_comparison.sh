...
source_data="${SOURCE_DATA:-$repo_root/experiments/rl-framework-comparison/math-smoke.jsonl}"
data_dir="${DATA_DIR:-$repo_root/data/rl-framework-comparison/verl}"
output_dir="${OUTPUT_DIR:-/mnt/post-training/rl-framework-comparison/verl}"

[[ -x "$python_bin" ]] || { echo "missing Verl Python: $python_bin" >&2; exit 2; }
[[ -f "$model_dir/config.json" ]] || { echo "missing model: $model_dir" >&2; exit 2; }
[[ -f "$source_data" ]] || { echo "missing data: $source_data" >&2; exit 2; }
mkdir -p "$output_dir/logs"
export PATH="$(dirname "$python_bin"):$PATH"
export NCCL_SOCKET_IFNAME="${NCCL_SOCKET_IFNAME:-enp1s0f0np0}"
export NCCL_IB_HCA="${NCCL_IB_HCA:-rocep1s0f0}"
"$python_bin" -c 'import ray; ray.init(address="auto"); assert int(ray.cluster_resources()["GPU"]) == 2; ray.shutdown()'

export PYTHONPATH="$repo_root/backends/verl${PYTHONPATH:+:$PYTHONPATH}"
"$python_bin" -m verl_lab.prepare_comparison "$source_data" "$data_dir"
...
exec "$python_bin" -m verl.trainer.main_ppo \
  ...
  reward.custom_reward_function.path="$repo_root/backends/verl/verl_lab/comparison_reward.py" \
